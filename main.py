...
@app.get("/direct-preview/{file_id}")
async def get_direct_preview_url(
    file_id: str,
    use_personal_drive: bool = False,
    drive_id: str = "drive_1",
    ...
    try:
        ...
        direct_preview_url = f"https://drive.google.com/file/d/{file_id}/preview"
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get direct preview URL: {str(e)}")
...
async def reject_share_request(share_id: str, current_user: str = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to reject share request")from fastapi import FastAPI, HTTPException, Depends, UploadFile, File, Form, WebSocket, Request
from fastapi.security import HTTPBearer, HTTPAuthorizationCredentials
from fastapi.middleware.cors import CORSMiddleware
from fastapi.responses import RedirectResponse, StreamingResponse, HTMLResponse
from fastapi.websockets import WebSocketDisconnect
